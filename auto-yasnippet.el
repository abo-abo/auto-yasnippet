...
;; just call `create-auto-yasnippet' (from anywhere on this line).
...
(defvar *auto-yasnippet-field-regex* "\\([a-z0-9-_]+\\)")
...
Removes `*auto-yasnippet-marker*' prefixes,
writes the corresponding snippet to `*current-auto-yasnippet-template*',
with words prefixed by `*auto-yasnippet-marker*' as fields, and mirrors properly set up."
  (interactive)
  (multiple-value-bind (s head tail) (current-string-position)
    (flet ((parse (in vars out)
		  (if in
		      (let ((p (string-match (concat
					      *auto-yasnippet-marker*
					      *auto-yasnippet-field-regex*) in)))
			(if p
      ...
      (insert (replace-regexp-in-string *auto-yasnippet-marker* "" s)))))
